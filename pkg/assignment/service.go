...
func (s *Service) Create(ctx context.Context, a *m.Assignment) (*m.Assignment, error) {
    if a.TitleAR == "" { return nil, Err("title required") }
    if a.DueAt.IsZero() { return nil, Err("due_at required") }
...
func (s *Service) Get(ctx context.Context, id string) (*m.Assignment, error) { return s.repo.Get(ctx, id) }

func (s *Service) Submit(ctx context.Context, sub *m.AssignmentSubmission) (*m.AssignmentSubmission, error) {
    if sub.AssignmentID == "" || sub.StudentID == "" { return nil, Err("assignment_id and student_id required") }
